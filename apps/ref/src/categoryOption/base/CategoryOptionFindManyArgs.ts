/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CategoryOptionWhereInput } from "./CategoryOptionWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { CategoryOptionOrderByInput } from "./CategoryOptionOrderByInput";

@ArgsType()
class CategoryOptionFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => CategoryOptionWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => CategoryOptionWhereInput, { nullable: true })
  @Type(() => CategoryOptionWhereInput)
  where?: CategoryOptionWhereInput;

  @ApiProperty({
    required: false,
    type: [CategoryOptionOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [CategoryOptionOrderByInput], { nullable: true })
  @Type(() => CategoryOptionOrderByInput)
  orderBy?: Array<CategoryOptionOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { CategoryOptionFindManyArgs as CategoryOptionFindManyArgs };
